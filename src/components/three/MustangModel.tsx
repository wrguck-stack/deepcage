...
const FRONT_ROTATION_Y = -BASE_ROTATION_Y
const AUTO_ROTATION_SPEED = .24
...
export function MustangModel({ reduced, assembled }: Props) {
  ...
  useLayoutEffect(() => {
    ...
    rotationRoot.current?.updateMatrixWorld(true)
  ...
  useFrame((state, delta) => {
    ...
    if (rotationRoot.current) {
      if (reduced || assembled) rotationRoot.current.rotation.y = THREE.MathUtils.damp(rotationRoot.current.rotation.y, FRONT_ROTATION_Y, 3.5, delta)
      else rotationRoot.current.rotation.y += AUTO_ROTATION_SPEED * delta
    }
    if (modelRoot.current) {
      modelRoot.current.rotation.y = THREE.MathUtils.damp(modelRoot.current.rotation.y, BASE_ROTATION_Y, 3, delta)
      modelRoot.current.rotation.x = THREE.MathUtils.damp(modelRoot.current.rotation.x, 0, 3, delta)
    }
  ...
  return <group ref={rotationRoot}><group ref={modelRoot} position={[.25, -.05, 0]} rotation={[0, BASE_ROTATION_Y, 0]}><primitive object={prepared.model} /></group></group>
}
